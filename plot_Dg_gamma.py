...
''' plot Dg_n(gamma) as a function of gamma '''
...
plt.subplots_adjust(left=.05, right=.95, bottom=.13, top=.95)
...
num_minus_axes = plt.axes([.77, .03, .08, .05])
num_minus_button = Button(num_minus_axes, '-1')
num_plus_axes = plt.axes([.87, .03, .08, .05])
num_plus_button = Button(num_plus_axes, '+1')
# number of points
num_points_axes = plt.axes([.67, .03, .08, .05])
